...
import InviteStats from "../types/interface/InviteStats";
import InviteResults from "../types/interface/InviteResults";
import BonusResults from "../types/interface/BonusResults";
import {Snowflake} from "discord.js";

export default {
    async getInviter(user: Snowflake, guild: Snowflake): Promise<Snowflake> {
        return new Promise<Snowflake>((resolve, reject) => {
            connection.mysql.query(`SELECT * FROM \`invites\` WHERE \`user\` = "${user}" AND \`guild\` = "${guild}" AND \`inactive\` = "0"`, (error: Error, results: Array<InviteResults> | undefined) => {
    ...
    async setInviter(user: Snowflake, inviter: Snowflake, guild: Snowflake): Promise<undefined> {
        return new Promise<undefined>((resolve, reject) => {
            connection.mysql.query(`SELECT * FROM \`leaves\` WHERE \`user\` = "${user}" AND \`guild\` = "${guild}" AND \`inactive\` = "0"`, (error: Error, results: Array<InviteResults>) => {
                if (error) reject(error.message);
                if (results && results[0]) {
                    connection.mysql.query(`UPDATE \`leaves\` SET \`inactive\` = "1" WHERE \`id\` = "${results[0].id}"`, (error1: Error) => {
                        if (error1) reject(error1.message);
                    });
                }
                connection.mysql.query(`INSERT INTO \`invites\` (\`user\`, \`guild\`, \`inviter\`, \`inactive\`) VALUES ("${user}", "${guild}", "${inviter}", "0")`, (error1: Error) => {
                    if (error1) reject(error1.message);
                    resolve(undefined);
                });
            });
    ...
    async removeInvite(user: Snowflake, inviter: Snowflake, guild: Snowflake): Promise<undefined> {
        return new Promise<undefined>((resolve, reject) => {
            connection.mysql.query(`SELECT * FROM \`invites\` WHERE \`user\` = "${user}" AND \`guild\` = "${guild}" AND \`inactive\` = "0"`, (error: Error, results: Array<InviteResults>) => {
                if (error) reject(error.message);
                if (results[0]) {
                    connection.mysql.query(`UPDATE \`invites\` SET \`inactive\` = "1" WHERE \`id\` = "${results[0].id}"`, (error1: Error) => {
                        if (error1) reject(error1.message);
                    });
                }
                connection.mysql.query(`INSERT INTO \`leaves\` (\`user\`, \`guild\`, \`inviter\`, \`inactive\`) VALUES ("${user}", "${guild}", "${inviter}", "0")`, (error1: Error) => {
                    if (error1) reject(error1.message);
                    resolve(undefined);
                });
            });
